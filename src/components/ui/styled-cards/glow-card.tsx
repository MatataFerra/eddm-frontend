...
export function GlowCardImage({
  ...
  if (placement === "center") {
    return (
      <motion.div
        style={{ filter: blur }}
        className="relative w-full h-full overflow-hidden rounded-2xl sm:rounded-3xl">
...
export function GlowCardContent({
  ...
  return <div className={cn("mb-4 sm:mb-5 md:mb-6", className)}>{children}</div>;
}

export function GlowCardTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-xl sm:text-2xl md:text-3xl font-light tracking-tight text-white mb-1 leading-tight">
      {children}
    </h3>
  );
}

export function GlowCardSubtitle({ children }: { children: React.ReactNode }) {
  return <p className="text-sm sm:text-base text-white/50 font-light">{children}</p>;
}
